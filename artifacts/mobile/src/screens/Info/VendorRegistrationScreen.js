...
import { PATHS, dbPush } from '../../services/rtdb';
...
export default function VendorRegistrationScreen({ navigation }) {
  ...
  const onSubmit = async () => {
    ...
    try {
      const registrationId = Date.now().toString(36) + Math.random().toString(36).slice(2, 6);
      ...
      await dbPush(PATHS.vendorRegistrations, { ...payload, id: registrationId });
