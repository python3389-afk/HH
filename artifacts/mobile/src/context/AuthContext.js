...
const profileCacheKey = (uid) => `@orderme_profile_${uid}`;

async function saveProfileLocally(uid, appUser) {
  try {
    await AsyncStorage.setItem(profileCacheKey(uid), JSON.stringify(appUser));
  } catch (_) {}
}

async function loadProfileLocally(uid) {
  try {
...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const authUnsub = onAuthStateChanged(auth, (firebaseUser) => {
      ...
      loadProfileLocally(uid).then((cached) => {
        if (cached) {
          setUser(cached);
      ...
      profileUnsub = dbListen(PATHS.userProfile(uid), (profile) => {
        ...
        const next = buildAppUser(firebaseUser, profile || {});
        setUser(next);
        saveProfileLocally(uid, next);
      });
    });
  ...
  const updateProfileData = async (updates) => {
    ...
    saveProfileLocally(uid, nextUser);
